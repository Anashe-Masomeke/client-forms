...
Requirements:
    pip install google-auth google-auth-httplib2 google-api-python-client requests --trusted-host pypi.org --trusted-host files.pythonhosted.org --trusted-host pypi.python.org
...
def check_and_apply_update():
    ...
    def _do_download():
        try:
            ...
            if size < MIN_SIZE:
                ...
                raise Exception(f"Download seems incomplete ({size // 1024} KB).")
            bat_lines = [
                "@echo off", "ping 127.0.0.1 -n 6 > nul",
                f'start "" "{new_exe_path}"',
                "ping 127.0.0.1 -n 2 > nul", 'del "%~f0"',
            ]
    ...
    if error_holder[0]:
        ...
        messagebox.showerror("Update Failed",
            f"Could not update:\n\n{error_holder[0]}\n\n"
            f"Please download manually from:\ngithub.com/{GITHUB_USER}/{GITHUB_REPO}/releases/latest")
        err_root.destroy(); return
...
class SheetsSetupDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Label(body, text="The .json file downloaded from Google Cloud Console",
                 bg=BG, fg="#607080", font=("Segoe UI", 8)).pack(anchor="w", pady=(0, 4))
        key_row = tk.Frame(body, bg=BG); key_row.pack(fill="x", pady=(0, 10))
        ...
        btn_bar = tk.Frame(body, bg=BG); btn_bar.pack(fill="x", pady=(4, 0))
    ...
    def _save(self):
        ...
        if not sid:
            self._lbl_err.config(text="❌  Please paste your Google Sheet ID."); return
        if not key:
            self._lbl_err.config(text="❌  Please select the JSON key file."); return
        if not os.path.exists(key):
            self._lbl_err.config(text=f"❌  File not found: {key}")
            messagebox.showerror("File Not Found",
                                 f"Cannot find the JSON file at:\n{key}\n\nPlease browse to it again.",
                                 parent=self); return
...
class LoginDialog(tk.Tk):
    ...
    def _build(self):
        ...
        if saved_name:
            ...
            tk.Label(greet, text="Not you? Clear your name in Settings after login.",
                     bg="#0D2B4E", fg="#607080",
        ...
        self._eye = tk.Button(pw_row, text="👁", command=self._toggle,
                              ...
                              activebackground=FBC_MID, activeforeground=WHITE, padx=6)
    ...
    def _attempt(self):
        ...
        if not sender:
            self._lbl_err.config(text="❌  Please enter your name."); return
    ...
    def _close(self):
        self.authenticated = False; self.destroy()
...
class SendFormDialog(tk.Toplevel):
    ...
    def _close(self):
        self._unbind_scroll(); self.destroy()
    ...
    def _build(self):
        ...
        tk.Label(sender_strip, text=f"✏  Sending as:  {self.sender_name}",
    ...
    def _on_type_change(self):
        t = self._ftype.get()
        if t == "VFEX":
            self._row_vfex.pack(fill="x", pady=3); self._row_zse.pack_forget()
            tmpl = BODY_VFEX
        elif t == "ZSE":
            self._row_vfex.pack_forget(); self._row_zse.pack(fill="x", pady=3)
            tmpl = BODY_ZSE
        else:
            self._row_vfex.pack(fill="x", pady=3); self._row_zse.pack(fill="x", pady=3)
            tmpl = BODY_BOTH
        ...
        if t in ("VFEX", "BOTH"):
            ...
            if not vp or not os.path.exists(vp):
                messagebox.showwarning("No VFEX form",
                    "Please click '📂 Override' to attach the VFEX PDF.", parent=self); return
            attachments.append(vp)
        if t in ("ZSE", "BOTH"):
            zp = self._zse.get().strip()
            if not zp or not os.path.exists(zp):
                messagebox.showwarning("No ZSE form",
                    "Please click '📂 Override' to attach the ZSE PDF.", parent=self); return
...
class ReminderDialog(tk.Toplevel):
    ...
    def _send(self):
        ...
        ok, err = open_outlook(self.client["email"], self._subj.get().strip(), body)
        if not ok:
            messagebox.showerror("Outlook error", err, parent=self); return
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        metrics_row = tk.Frame(metric_bg, bg=SIDEBAR_BG); metrics_row.pack(padx=20)
        ...
        sb = tk.Scrollbar(list_outer, orient="vertical", command=self._canvas.yview)
    ...
    def _client_card(self, parent, client):
        ...
        info = tk.Frame(card, bg=CARD_BG); info.pack(side="left", fill="x", expand=True)
        ...
        right = tk.Frame(card, bg=CARD_BG); right.pack(side="right")
        if not done:
...
if __name__ == "__main__":
    ...
    settings = load_settings()
    print(f"[LAUNCH] key_file = {settings.get('key_file')}")
    print(f"[LAUNCH] sheet_id = {settings.get('sheet_id')}")
